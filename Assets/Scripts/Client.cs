...
public class Client
{
    ...
    public void SendIntoGame(string _playerName,Vector3 position,Quaternion rotation)
    {
        bool firstPlayer = false;
        if (!NetworkManager.instance.firstPlayer)
        {
            firstPlayer = true;
            player = NetworkManager.instance.InstantiatePlayer(position, rotation);
            NetworkManager.instance.firstPlayer = true;
        }
        else
        {
            Vector3 mirrorPosition = MirrorConverter.MirrorVector3(position, Axis.yAxis);
            Quaternion mirrorRotation = Quaternion.Euler(rotation.eulerAngles.x, (rotation.eulerAngles.y + 180f) % 360f, rotation.eulerAngles.z);
            player = NetworkManager.instance.InstantiatePlayer(mirrorPosition, mirrorRotation);
        }
        player.Initialize(id, _playerName,firstPlayer);
        ...
        foreach (Client _client in Server.clients.Values)
        {
            if (_client.player != null)
            {
                if (_client.id != id)
                {
                    ServerSend.SpawnPlayer(id, _client.player,position,rotation);
                }
            }
        }
        ...
        foreach (Client _client in Server.clients.Values)
        {
            if (_client.player != null)
            {
                ServerSend.SpawnPlayer(_client.id, player, position, rotation);
            }
        }
