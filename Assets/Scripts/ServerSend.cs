...
public class ServerSend
{
    ...
    public static void SpawnPlayer(int _toClient, Player _player, Vector3 position, Quaternion rotation)
    {
        using (Packet _packet = new Packet((int)ServerPackets.spawnPlayer))
        {
            ...
            {
                _packet.Write(MirrorConverter.MirrorVector3(position, Axis.yAxis));
                _packet.Write(Quaternion.Euler(rotation.eulerAngles.x, (rotation.eulerAngles.y + 180f) % 360f, rotation.eulerAngles.z));
            }
            else
            {
    ...
    public static void CanonRotation(int id,Vector3 RequriedVelocity)
    {
        using (Packet _packet = new Packet((int)ServerPackets.CanonRotation))
        {
            _packet.Write(id);
            _packet.Write(MirrorConverter.MirrorVector3(RequriedVelocity, Axis.yAxis));
    ...
    public static void CanonShoot(int id, Vector3 RequriedVelocity, int weaponIndex)
    {
        using (Packet _packet = new Packet((int)ServerPackets.CanonShoot))
        {
            _packet.Write(id);
            _packet.Write(MirrorConverter.MirrorVector3(RequriedVelocity, Axis.yAxis));
